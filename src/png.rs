...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        ...
        let mut i = 8;
        while i < bytes.len() - 1 {
            buffer.copy_from_slice(&bytes[i..(i + 4)]);
            let data_length = u32::from_be_bytes(buffer) as usize;
            // Chunk layout: Length(4-byte) + Chunk Type(4-byte) + Chunk Data(length-byte) + CRC(4-byte)
            let chunk_stop = i + 4 + 4 + data_length + 4;
            let chunk = Chunk::try_from(&bytes[i..chunk_stop])?;
            chunks.push(chunk);
            i = chunk_stop;
        }
        Ok(Png { chunks })
